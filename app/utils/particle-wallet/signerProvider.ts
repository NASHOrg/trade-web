...
import { convertSignature, pubKeyToEVMAddress } from './ethereumUtils';
// import { convertSignature, pubKeyToEVMAddress } from './ethereumUtils'
...
export class SignerProvider {
  ...
  async request(arg: RequestArguments) {
    ...
    if (arg.method === 'eth_accounts' || arg.method === 'eth_requestAccounts') {
      ...
      if (
        arg.params
        && arg.params instanceof Array
        && arg.params[0]
        && arg.params[0].chainId
      ) {
        ...
        throw new Error(`The chain: ${chainId} is not supported`);
      }
      else {
  ...
  personalSign = async (_message: string): Promise<string> => {
